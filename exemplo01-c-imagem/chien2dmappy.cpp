...
void Chien2DMappy::desenhaMapa(unsigned int idAzulejo, double x, double y)
{
    // Desenha todas as camadas do mapa
    for(int i=0;i<MAX_CAMADAS && this->camadas[i]!=0;i++)
        desenhaCamadaMapa(idAzulejo, i, x, y);
}
...
void Chien2DMappy::desenhaCamadaMapa(unsigned int idAzulejo, int camada, double x, double y)
{
    ...
    for(int linha=0;linha<alturaAzulejos;linha++)
        for(int coluna=0;coluna<larguraAzulejos;coluna++)
        {
            ...
            if(azulejo!=0)
            {
                renderer->desenhaAzulejo(idAzulejo, azulejo-1, 
